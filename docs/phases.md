...
| 10: Codex plugin | Pacote portátil e marketplace gerados; instalação no Codex Windows com ponte WSL e descoberta MCP confirmadas. Chamada dentro de uma nova tarefa ainda precisa de verificação. |
...
| 16: outros hosts | Plugin para Google Antigravity empacotado conforme documentação oficial; host não instalado neste ambiente para teste operacional. |

Ainda faltam evidências de economia real, avaliação robusta para coding/reasoning, verificação operacional em Claude Code e Antigravity, publicação npm e operação SaaS. O repositório está publicado no GitHub. O produto permanece em fallback por padrão até um experimento passar os gates para o modelo e a tarefa concretos.
