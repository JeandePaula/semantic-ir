...
O `BudgetLedger` reserva o máximo de tokens de output configurado antes de cada inferência e para quando o teto de requests, tokens, custo calculado ou tempo é atingido. O teto em USD é uma estimativa baseada nos preços informados pelo usuário, não uma garantia de cobrança do provider. Cached input é subconjunto de input. Para OpenAI, reasoning é subconjunto de output; outros providers podem reportar detalhes com outra semântica. Não somar esses campos novamente sem confirmar o contrato do provider. O custo de calibração aparece no relatório da execução, separado das métricas persistidas de inferência do usuário.
